...
//    private FloatingActionButton fabCall, fabLike, fabBlock, fabShortlist, fabMessage, fabSendInterest;
    ...
    private ImageView fabAudioCall, fabVideoCall;

    private int callType = 1; // 1 = video & 2 = audio
    private long callSeconds = 0;
    private boolean isCallTimeFinish = false;

    private ZegoSendCallInvitationButton newVideoCall;
    private ZegoSendCallInvitationButton newVoiceCall;

    private LinearLayout layoutViewVideo;
    private String videoLink = "";

    private LinearLayout layVerify;
    private ImageView imgIdProofVerify,imgChurchVerify;

//    ImageView badgeIdVerified, badgeChurchVerified;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_other_user_profile_old);
        ...
        layoutViewVideo = findViewById(R.id.layoutViewVideo);

        scrollView = findViewById(R.id.scrollView);
        txtFocus = findViewById(R.id.txtFocus);

//        badgeIdVerified = findViewById(R.id.badgeIdVerified);
//        badgeChurchVerified = findViewById(R.id.badgeChurchVerified);
        ...
        fabAudioCall = findViewById(R.id.fabAudioCall);
        ...
        imgIdProofVerify = findViewById(R.id.imgIdProofVerify);
        imgChurchVerify = findViewById(R.id.imgChurchVerify);
        ...
        fabAudioCall.setOnClickListener(this);
        fabVideoCall.setOnClickListener(this);

        btnMore.setOnClickListener(this);
        layoutViewVideo.setOnClickListener(this);
...
//                    if (dataObject.get("video_approval").getAsString().equalsIgnoreCase("APPROVED")
//                            && !dataObject.get("video_url").getAsString().equalsIgnoreCase("")
//                            && dataObject.get("membership_video_status").getAsString().equalsIgnoreCase("Yes")) {
//                        layoutViewVideo.setVisibility(View.VISIBLE);
//                        videoLink = dataObject.get("video_url").getAsString();
//                    } else {
//                        layoutViewVideo.setVisibility(View.GONE);
//                    }

//                    String id_proof = dataObject.get("id_proof").getAsString();
//                    String id_proof_approve = dataObject.get("id_proof_approve").getAsString();

//                    if (!id_proof.isEmpty() && id_proof_approve.equalsIgnoreCase("APPROVED")) {
//                        badgeIdVerified.setVisibility(View.VISIBLE);
//                    } else {
//                        badgeIdVerified.setVisibility(View.GONE);
//                    }
//
//                    if (dataObject.has("church_verified")) {
//                        String church_verified = dataObject.get("church_verified").getAsString();
//                        if (!church_verified.isEmpty() && church_verified.equalsIgnoreCase("Yes")) {
//                            badgeChurchVerified.setVisibility(View.VISIBLE);
//                        } else {
//                            badgeChurchVerified.setVisibility(View.GONE);
//                        }
//                    }

//                    layVerify.setVisibility(View.VISIBLE);
//                    Picasso.get().load(data.getString("idProofImagePath")).into(imgIdProofVerify);
//                    Picasso.get().load(data.getString("churchVerifiedUnverifiedPath")).into(imgChurchVerify);
                    ...
                    for (ViewProfileSectionBean viewProfileSectionBean : userProfileDataList) {
                        ...
                        if(viewProfileSectionBean.getViewProfileFieldList().size() > 0){
    ...
    private void updateAudioVideoCallDuration(long seconds, int callType) {
        if(seconds == 0) return;
        ZegoUIKitPrebuiltCallInvitationService.removeInvitationCallListener();
        common.showProgressRelativeLayout(loader);

        HashMap<String, String> param = new HashMap<>();
        param.put("member_id", session.getLoginData(SessionManager.KEY_USER_ID));
        param.put("callType", String.valueOf(callType));
        param.put("seconds", String.valueOf(seconds));
        common.makePostRequest(AppConstants.UPDATE_VIDEO_AUIDO_CALL_DURATION, param, response -> {
            AppDebugLog.print("profile response in other user profile : " + response);
            common.hideProgressRelativeLayout(loader);
            try {
                JSONObject object = new JSONObject(response);
                ...
                ApplicationData.callType = 0;
                ApplicationData.totalCallDuration = 0;

                AppDebugLog.print("in UPDATE_VIDEO_AUIDO_CALL_DURATION isCallByMe : " + ApplicationData.isCallByMe);
                AppDebugLog.print("in UPDATE_VIDEO_AUIDO_CALL_DURATION callType : " + ApplicationData.callType);
                AppDebugLog.print("in UPDATE_VIDEO_AUIDO_CALL_DURATION totalCallDuration : " + ApplicationData.totalCallDuration);

                callSeconds = 0;
                if(isCallTimeFinish){
                    isCallTimeFinish = false;
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btnMore:
                PopupMenu popup = new PopupMenu(OtherUserProfileActivity.this, btnMore);
...
//                            intent.putExtra("is_voice_call", true);
//                            intent.putExtra("other_matri_id", other_matri_id);
//                            startActivity(intent);
                        //     break;
                        // case R.id.action_video_call:
//                            Intent intentV = new Intent(this, VideoCallActivity.class);
//                            intentV.putExtra("is_video_call", true);
//                            intentV.putExtra("other_matri_id", other_matri_id);
//                            startActivity(intentV);
                ...
                popup.show();
                break;
            case R.id.layoutViewVideo:
                //display video
//                Intent i = new Intent(OtherUserProfileActivity.this, VideoPlayerActivity.class);
//                i.putExtra("video_link", videoLink);
//                startActivity(i);
            ...
            case R.id.fabAudioCall:
                if (ApplicationData.isAllowAudioCall) {
                    callType = 2;
                    newVoiceCall.performClick();
                } else {
                    AlertDialog.Builder alertConfirmViewContactDetails = new AlertDialog.Builder(this);
                    alertConfirmViewContactDetails.setTitle("Update Membership");
                    alertConfirmViewContactDetails.setMessage("This Feature is not available in your plan, Please upgrade your membership plan.");
                    alertConfirmViewContactDetails.setPositiveButton("Upgrade", (dialogInterface, i) -> {
                        startActivity(new Intent(this, PlanListActivity.class));
                    });
                    AlertDialog alert = alertConfirmViewContactDetails.create();
                    alert.show();
            ...
            case R.id.fabVideoCall:
                AppDebugLog.print("sgksdgsjdf" +ApplicationData.isAllowVideoCall);
                if (ApplicationData.isAllowVideoCall) {
                    callType = 1;
    ...
    public void lastSectionExpand(ViewProfileSectionBean section) {
        if (section.getId().equalsIgnoreCase("contact_info") || section.getId().equalsIgnoreCase("family_info")) {
    ...
    public void initCallInviteService() {
        ZegoUIKitPrebuiltCallInvitationService.addInvitationCallListener(new ZegoInvitationCallListener() {
            ...
            public void onIncomingCallReceived(String callID, ZegoCallUser caller, ZegoCallType callType, List<ZegoCallUser> callees) {
                AppDebugLog.print("in onIncomingCallReceived1");
            }
            ...
            public void onOutgoingCallAccepted(String callID, ZegoCallUser callee) {
                AppDebugLog.print("in onIncomingCallReceived2");
                ...
                config.durationConfig.durationUpdateListener = seconds -> {
                    callSeconds = seconds;
...
//                ZegoUIKitPrebuiltCallInvitationService.getPrebuiltCallFragment().setPrebuiltCallConfig(config);
            }
        ...
        ZegoUIKitPrebuiltCallInvitationService.addOutgoingCallButtonListener(() -> {
            AppDebugLog.print("in onIncomingCallReceived3");
        });
        ZegoUIKitPrebuiltCallInvitationService.addIncomingCallButtonListener(new IncomingCallButtonListener() {
            ...
            public void onIncomingCallDeclineButtonPressed() {
                AppDebugLog.print("in onIncomingCallReceived4");
            }

            @Override
            public void onIncomingCallAcceptButtonPressed() {
                AppDebugLog.print("in onIncomingCallReceived5");
            }
        });
    ...
    private void initVideoButton() {
        ...
        newVideoCall.setOnClickListener(v -> {
            String[] split = other_matri_id.split(",");
    ...
    protected void onResume() {
        ...
        AppDebugLog.print("in onResume isCallByMe : " + ApplicationData.isCallByMe);
        AppDebugLog.print("in onResume callType : " + ApplicationData.callType);
        AppDebugLog.print("in onResume totalCallDuration : " + ApplicationData.totalCallDuration);

        if (ApplicationData.isCallByMe && ApplicationData.totalCallDuration > 0) {
            updateAudioVideoCallDuration(ApplicationData.totalCallDuration, ApplicationData.callType);
        }

        //AppDebugLog.print("in onResume" + ZegoUIKitPrebuiltCallInvitationService.getPrebuiltCallFragment());

//        if (ZegoUIKitPrebuiltCallInvitationService.getPrebuiltCallFragment() != null) {
//            updateAudioVideoCallDuration(callSeconds, callType);
//        }
    }
    ...
    protected void onPause() {
        super.onPause();
        AppDebugLog.print("in onPause" + ZegoUIKitPrebuiltCallInvitationService.getPrebuiltCallFragment());
    }
}
